...
function getItemRect1(item, model) {
    let radiant = (Math.PI * 2) * (item.value / model.total)
    return {
        rad: radiant,
        cx: !item.active ? CANVAS_WIDTH / 2 + CANVAS_HEIGHT / 2.8*Math.cos((model.start + model.start + radiant)/2) : CANVAS_WIDTH / 2 + (CANVAS_HEIGHT / 2.8 + CANVAS_HEIGHT / 9)*Math.cos((model.start + model.start + radiant)/2),
        cy: !item.active ? CANVAS_WIDTH / 2 + CANVAS_HEIGHT / 2.8*Math.sin((model.start + model.start + radiant)/2) : CANVAS_WIDTH / 2 + (CANVAS_HEIGHT / 2.8 + CANVAS_HEIGHT / 9)*Math.sin((model.start + model.start + radiant)/2),
        x: !item.active ? CANVAS_WIDTH / 2 : CANVAS_WIDTH / 2 + CANVAS_HEIGHT / 9*Math.cos((item.start + item.start + radiant)/2),
        y: !item.active ? CANVAS_HEIGHT / 2 : CANVAS_WIDTH / 2 + CANVAS_HEIGHT / 9*Math.sin((item.start + item.start + radiant)/2),
    };
}
...
function animate1() {
    if (model.prevCur != model.degCur || model.isRecalculation == true) {
